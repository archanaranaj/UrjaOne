    ...
    return (
      <div style={{ padding: 20 }}>
        <Button
          ...
          onClick={() => navigate(-1)}
        />
        <p>No specification record found.</p>
      </div>
    );
  }
    ...
    <Space direction="vertical" size="large" style={{ width: "100%", padding: 0 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <Button
          ...
          onClick={() => navigate(-1)}
        />
        <Title level={4} style={{ margin: 0 }}>View Vendor Service Specification</Title>
      </div>

      {/* Summary */}
      <Card bodyStyle={{ paddingTop: 16 }}>
        <Title level={5} style={{ marginBottom: 0 }}>{record.name}</Title>
        <p style={{ marginBottom: 0 }}>Vendor Service ID: {record.vendorServiceId}</p>
      </Card>
      ...
      >
        <Descriptions bordered column={1} size="small">
          ...
          <Descriptions.Item label="Specs ID">{record.specsId}</Descriptions.Item>
          <Descriptions.Item label="Specs Name">{record.specsName}</Descriptions.Item>
          <Descriptions.Item label="Specs Value">{record.specsValue}</Descriptions.Item>
