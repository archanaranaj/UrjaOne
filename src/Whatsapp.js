...
//   {
//     key: 1,
//     whatsappApi: "https://yourdomain.com/api/whatsapp/initiate",
...
//         item.whatsappApi.toLowerCase().includes(value.toLowerCase())
//     );
...
//     const newEntry = {
//       key: newKey,
//       whatsappApi: values.whatsappApi,
...
//     setIsModalVisible(false);
//     message.success("Whatsapp API added successfully.");
//     form.resetFields();
//   };
...
//             <span style={{ fontSize: 20, fontWeight: 600 }}>Whatsapp API</span>
//             <Space>
//               <Input
...
//               name="whatsappApi"
//               label="Whatsapp API"
//               rules={[{ required: true, message: "Please enter Whatsapp API" }]}
...
const Whatsapp = () => {
  ...
  const handleEdit = (record) => {
    ...
    form.setFieldsValue({
      whatsappApi: record.whatsappApi,
    });
  };
  ...
  const handleFormSubmit = (values) => {
    if (isEditMode && editingKey !== null) {
      ...
      setData(updatedData);
      setFilteredData(updatedData);
      message.success("Whatsapp API updated successfully.");
    } else {
      // Add new entry
      const newKey = data.length + 1;
      const newEntry = {
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      ...
      >
        ...
        <Modal
          title={isEditMode ? "Edit Whatsapp API" : "Add Whatsapp API"}
          open={isModalVisible}
          onCancel={handleModalCancel}
          onOk={() => form.submit()}
          okText="Submit"
